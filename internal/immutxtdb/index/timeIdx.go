...
func NewCreationTimeIndex(indexDir, device, salt string) (DocByTimeIndex, error) {
	...
	return idx.NewBasicIndex(indexDir, CreationTimeIdxQualifier, device, keySer, valSer, nil, idx.NewRotatingHasher([]byte(salt), TimeIdxKeySize), enc, TimeIdxPageSize)
}

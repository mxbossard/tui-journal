...
func TestBucketService_ProjectText(t *testing.T) {
	...
	err = bkt1.SetText(expectedMsg3)
	assert.NoError(t, err)
	err = svc.Save(bkt1, expectedPartition)
	assert.NoError(t, err)

	err = bkt1.SetText(expectedMsg4)
	assert.NoError(t, err)
	err = svc.Save(bkt1, expectedPartition)
	assert.NoError(t, err)
...
func TestBucketService_Filter(t *testing.T) {
	...
	// Update bkt1
	bkt1.SetText(expectedMsg1 + "updated")
	err = svc.Save(bkt1, expectedPartition, day3)
	assert.NoError(t, err)

	// Update bkt2
	bkt2.SetText(expectedMsg2 + "updated")
	err = svc.Save(bkt2, expectedPartition, day3)
	assert.NoError(t, err)
	...
	// Update bkt4
	bkt4.SetText(expectedMsg4 + "updated")
...
func TestBucketService_ConcurrentSave_SameServiceSamePart_MustConflict(t *testing.T) {
	tmpDir := filez.MkdirTempOrPanic(t.Name())
	defer os.RemoveAll(tmpDir)
	...
	expectedName := "foo"
	expectedMsg1 := ztring.LoremIpsumWords(10)
	expectedMsg1a := expectedMsg1 + "updateA"
	expectedMsg1b := expectedMsg1 + "updateB"
	expectedLabels := Labels{
		"foo": "pif",
		"bar": "paf",
	}
	...
	assert.Len(t, layersA, 2)
	// check layer versions
	require.True(t, len(layersA) >= 2)
	...
	assert.Equal(t, []error{nil, nil}, errs)
	assert.Len(t, layersC, 2)
	// check layer versions
	require.True(t, len(layersC) >= 2)
...
func TestBucketService_ConcurrentSave_TwoServicesTwoDirs_MustNotConflict(t *testing.T) {
	...
	// Get the bucket 1a
	bkt1c, err := svc1.Get(bkt1.Header.Uid)
	assert.NoError(t, err)
	require.NotNil(t, bkt1c)
	...
	// Get the bucket 1b
	bkt1d, err := svc2.Get(bkt1.Header.Uid)
	assert.NoError(t, err)
	require.NotNil(t, bkt1d)

	// Last version must be latest OK saved
	txt, err = bkt1d.ProjectText(LatestVersion)
	assert.NoError(t, err)
	assert.Equal(t, expectedMsg1b, txt)

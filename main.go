...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	//m.mdBrowser.Update(msg)
	if textareaUpdated {
		updateMsg := UpdateMsg{
			content: m.inputs[0].Value(),
		}
		//fmt.Printf("msg: %v\n", m.inputs[0].Value())
		m.mdBrowser.Update(updateMsg)
	} else {
		//m.mdBrowser.Update(msg)
	}
...
func (m *model) sizeInputs() {
	...
	m.mdBrowser.viewport.Width = (m.width / (len(m.inputs) + 1))
	m.mdBrowser.viewport.Height = m.height - helpHeight
}
...
func (m model) View() string {
	...
	views = append(views, m.mdBrowser.View())
